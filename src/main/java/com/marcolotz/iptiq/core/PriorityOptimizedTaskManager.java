...
public class PriorityOptimizedTaskManager implements TaskManager {
    ...
    public void addProcess(final Process process) {
        ...
        if (numberOfElements < capacity) {
            final Map<UUID, AcceptedProcessDecorator> prioQueue = taskQueues.getOrDefault(process.getPriority(), new LinkedHashMap<>());
            ...
            final Optional<Priority> priorityEntry = prioritiesInAscendingOrder.stream()
                .filter(
            ...
            if (priorityEntry.isPresent()) {
                ...
                final Map.Entry<UUID, AcceptedProcessDecorator> oldest = taskQueues.get(priority).entrySet().iterator().next();
    ...
    public List<Process> listRunningProcess(final SortingMethod method) {
        if (method.equals(SortingMethod.PRIORITY)) {
            final LinkedList<Process> list = new LinkedList<>();
    ...
    public void killProcess(final String pid) throws ProcessNotFoundException {
        final UUID inputPid = UUID.fromString(pid);
        final Optional<AcceptedProcessDecorator> processToKill = taskQueues.values()
            .stream()
