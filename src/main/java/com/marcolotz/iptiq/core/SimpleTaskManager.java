...
public class SimpleTaskManager implements TaskManager {
    ...
    public List<Process> listRunningProcess(final SortingMethod method) {
        final Comparator<AcceptedProcessDecorator> processComparator = method.comparator();
    ...
    public void killProcess(final String pid) throws ProcessNotFoundException {
        final UUID uuid = UUID.fromString(pid);
        final Optional<AcceptedProcessDecorator> process = tasks
            .stream()
    ...
    public void killGroup(final Priority priority) {
        final Set<AbstractProcess> killedProcesses = tasks.stream()
            .filter(p -> p.getPriority().equals(priority))
            .peek(AbstractProcess::kill)
            .collect(Collectors.toSet());
        final List<AcceptedProcessDecorator> aliveProcesses =
            tasks.stream().filter(p -> !killedProcesses.contains(p)).collect(Collectors.toList());
        tasks.clear();
